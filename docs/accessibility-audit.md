...
| Remediated | PDF highlights      | Selected single-region overlays now expose page-relative position and size through `aria-describedby`; the visual overlay is hidden from assistive technology.                                                                                                              | Multiple-highlight counts remain future work when multi-region viewer rendering is introduced. |
