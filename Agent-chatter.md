...
- 2026-09-09 — **Session workload update across agents.** Agent A owned export/PDF correctness follow-through: reference-line placement was corrected to a 3pt payee/reference gap, divider clearance now respects the full reference line, and the formatted text template editor was reconnected to the final PDF renderer for live parity. Agent B owned renderer workflow and UX: the Export Panel was thinned into Data Exports and Layout & PDF Studio, text/PNG canvas previews were unified into one Canvas & layout studio, the popup was enlarged, per-mode config buttons were restored, PNG placements became descriptive rows, and the left preview sidebar became a toolbar-driven options panel. Agent C owned interaction QA and review tooling: the Marks panel now applies scoped Keep/Maybe/Exclude entry changes, highlight geometry apply resolves current targets at mutation time, and focused renderer/export/typecheck gates were rerun after each change.
...
- 2026-09-05 follow-up — Reduced the default Payee / description row spacing by 3 points, from 28pt to 25pt, for newly created kept-export templates. Existing saved templates retain their configured spacing.
- 2026-09-05 follow-up — Reduced the default Payee / description row spacing by another 5 points, from 25pt to 20pt, for newly created kept-export templates. Existing saved templates retain their configured spacing.
- 2026-09-05 follow-up — Reworked the kept-entry canvas preview modes into explicit layer focus. `PNG layer`, `Text layer`, and `All layers` now dim rather than hide the inactive layer, so alignment context stays visible while only the active layer is selectable. Layer-specific tooling and per-layer counts replace the previous generic toggle. Canvas tests (18/18) and full typecheck pass.
- 2026-09-05 follow-up — Retuned the reference line spacing in the kept-text export plan to millimetre-based gaps: the reference now sits 2mm below the payee/description anchor (previously a 1pt offset) and reserves a 3mm clear gap beneath itself so row dividers no longer crowd it. Added `POINTS_PER_MM`, `REFERENCE_TOP_GAP_MM`, and `REFERENCE_BOTTOM_GAP_MM` constants in `src/export/keptExportLayout.ts`, plus a divider-clearance regression test. Export layout and template PDF tests (23/23) and full typecheck pass.
- 2026-09-06 - Agent C — Centred the payee/reference pair within its row band. The gap between payee and reference is now 5mm, and 5mm of clear space is reserved beneath the reference before the divider. Leftover row height is split evenly above the payee and below the reference so the pair reads as a centred block rather than sitting hard against the top of the band. Default column pitch for new templates raised from 20pt to 52pt, the minimum that physically accommodates two lines plus both 5mm gaps; saved templates keep their stored spacing. Export layout (7/7), template PDF (16/16), template draft (5/5) tests and full typecheck pass.
- 2026-09-06 follow-up — Reverted the centred 5mm payee/reference layout at request. Restored the default 20pt row pitch for newly created templates and removed the centring mutations. References now sit as close as possible beneath payee/description with a 1pt top offset; the existing 3mm clearance below the reference remains so dividers do not collide. Focused layout/template tests (12/12), Prettier, and full typecheck pass.
